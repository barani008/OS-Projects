...
struct trfs_sb_info {
	...
	struct trfs_kthread_ctl *ctl;
	struct task_struct *threadId;
	struct file * filp;
	struct lockref record_id;
};
...
/* getting and setting the trace file path */
static inline char * get_trfs_trace_file_path(
        const struct super_block *sb)
{
        return TRFS_SB(sb)->file_name;
}

static inline void set_trfs_trace_file_path(struct super_block *sb,
                                          char * name)
{
        TRFS_SB(sb)->file_name = name;
}
	
/* getting and setting the trace bitmap */
static inline unsigned long get_trfs_trace_bitmap(
        const struct super_block *sb)
{
        return TRFS_SB(sb)->bitmap;
}

static inline void set_trfs_trace_bitmap(struct super_block *sb,
                                          unsigned long bitmap)
{
        TRFS_SB(sb)->bitmap = bitmap;
}

static inline unsigned int get_trfs_trace_record_id(struct trfs_sb_info *sb)
{
         if (sb)
                 lockref_get(&sb->record_id);
         return sb->record_id.count;
}
